...
_size(size) {
	...
	int margin_around_plot = 10;
	int additional_space_for_pointer = 5;

	// setting axes and net
	_x_axis = sf::RectangleShape( sf::Vector2f(size.x - 2 * margin_around_plot + additional_space_for_pointer, 1) );
	_y_axis = sf::RectangleShape( sf::Vector2f(1, size.y - 2 * margin_around_plot + additional_space_for_pointer) );
	_x_axis.setFillColor(sf::Color::Black);
	_x_axis.setPosition(sf::Vector2f(margin_around_plot, height_whole - margin_around_plot));
	_y_axis.setFillColor(sf::Color::Black);
	_y_axis.setPosition(sf::Vector2f(margin_around_plot, margin_around_plot - additional_space_for_pointer));
	

	sf::RectangleShape net_line( sf::Vector2f(size.x - 2 * margin_around_plot, 1) );
	net_line.setFillColor( sf::Color(150, 150, 150) );
	int deltaY_px = (height_whole - 2 * margin_around_plot) / _amount_of_back_lines_x;
	int deltaX_px = (size.x - 2 * margin_around_plot) / _amount_of_back_lines_y;

	for (int i = 1; i <= _amount_of_back_lines_x; i++) {
		net_line.setPosition( sf::Vector2f(margin_around_plot, height_whole - margin_around_plot - i * deltaY_px) );
		_x_axis_back_lines.push_back( sf::RectangleShape(net_line) );
	}

	net_line.setSize( sf::Vector2f(1, size.y - 2 * margin_around_plot) );
	for (int i = 1; i <= _amount_of_back_lines_y; i++) {
		net_line.setPosition( sf::Vector2f(margin_around_plot + i * deltaX_px, margin_around_plot) );
...
std::pair<double, double> FunctionPlot2D::project_point( std::pair<double, double> fun_point ) const {
	int margin_around_plot = 10;
	double x = (_size.x - 2 * margin_around_plot) * (fun_point.first - _x_min) / (_x_max - _x_min) + margin_around_plot;
	double y = _size.y - ( (_size.y - 2 * margin_around_plot) * (fun_point.second - _y_min) / (_y_max - _y_min) + margin_around_plot );
